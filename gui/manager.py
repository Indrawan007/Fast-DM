...
.empty-label { color: #45475a; font-size: 13px; }
"""
...
class DownloadRow(Gtk.ListBoxRow):
    ...
    def __init__(self, dl_data):
        ...
        self.filename_lbl.set_markup(
            "<b>{}</b>".format(
                GLib.markup_escape_text(dl_data["filename"])
            )
        )
...
# Ganti SELURUH method update() di class DownloadRow dengan ini:
    ...
    def update(self, d):
        ...
        if status == "downloading":
            ctx.add_class("status-downloading")
        elif status == "completed":
            ctx.add_class("status-completed")
        elif status == "error":
...
class ManagerWindow(Gtk.Window):
    ...
    def add_download_from_extension(self, url, filename=None, headers=None):
        """Called from native-messaging listener (any thread)."""
    ...
    def _add_row(self, dl_id):
        """Add a DownloadRow — must run in GTK main thread."""
        ...
        row.pause_btn.connect(
            "clicked",
            lambda _b: self.engine.pause_download(dl_id)
        )
        row.resume_btn.connect(
            "clicked",
            lambda _b: self.engine.resume_download(dl_id)
        )
        row.cancel_btn.connect(
            "clicked",
            lambda _b: self.engine.cancel_download(dl_id)
        )
        row.open_btn.connect(
            "clicked",
            lambda _b, d=dl_data: self._open_folder(d)
        )
    ...
    def _on_clear_done(self, _widget):
        done_ids = [
            dl_id for dl_id, row in self._rows.items()
            if (d := self.engine.get_download(dl_id))
            and d["status"] in ("completed", "cancelled", "error")
        ]
        for dl_id in done_ids:
            row = self._rows.pop(dl_id)
            self.listbox.remove(row)
            self.engine.remove_download(dl_id)

    # ------------------------------------------------------------------
    # Engine callbacks  (may be called from worker threads)
    # ------------------------------------------------------------------

    def _cb_update(self, dl_data):
        GLib.idle_add(self._gtk_update_row, dl_data)

    def _cb_complete(self, dl_data):
        GLib.idle_add(self._gtk_update_row, dl_data)

    def _gtk_update_row(self, dl_data):
        """Run inside GTK main thread via idle_add."""
        row = self._rows.get(dl_data["id"])
        if row:
            row.update(dl_data)
        self._gtk_update_stats()

    def _gtk_update_stats(self):
