...
>
> **Catatan historis:** bagian analisis §1–§4 menggambarkan kode v2.2.5 sebelum
> perbaikan pada tabel §0. Untuk arsitektur yang sedang berlaku, gunakan
> `AGENTS.md` dan `README.md`; jangan menyalin path `/tmp`, model subprocess,
> atau perilaku restore lama dari bagian historis tersebut.
...
3. Saat `activate`: buat channel `mpsc::unbounded<DownloadEvent>`, `DownloadEngine::new`,
   lalu spawn **thread `fastdm-ipc`** yang menjalankan Unix-socket server dari
   `Config::ipc_socket_path()` (`$XDG_RUNTIME_DIR/fast-dm/fast-dm.sock`, fallback
   `~/.config/fast-dm/run/fast-dm.sock`, direktori 0700 + verifikasi peer UID),
   dan `gui::window::build_window`.
...
- **Session persist**: flusher tokio tiap ≤2 detik menulis `session.json`
  (tmp+rename atomik, file 0600, maks 200 entri terbaru). Saat restart,
  `Downloading/Resolving/Queued` → `Paused` dengan `pid=None`, lalu dilanjutkan
  otomatis bila `auto_resume` aktif.
...
3. **Spawn `aria2c`** lewat `tokio::process::Command` pada fallback per-proses:
   URL ditulis ke **input-file privat** `Config::aria2_input_dir()/aria2-<id>.txt`
   (direktori 0700, file 0600; aman untuk URL panjang), semua opsi lain via
   argumen CLI (`--max-connection-per-server`, `--split`,
...
**Kualitas kode & DX** 10. Unit dan integration test mencakup fungsi murni (parser, sanitizer, walk-up cookies,
roundtrip serde, char-boundary truncation UTF-8); jumlah sengaja tidak di-hardcode agar
dokumentasi tidak kembali drift saat regression test ditambahkan. 11. Komentar "mengapa" (bukan "apa") dengan kode bug (B1–B20) — jejak audit yang jelas;
...
1. **CI**: jadikan `cargo clippy --all-targets -- -D warnings` blocking setelah
   baseline bersih; tambah `cargo deny check` (advisories + license), `web-ext lint`
   untuk extension, job `apt install`→install .deb
