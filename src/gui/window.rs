...
/// v2.10.5: menerima `DownloadSummary` (snapshot ringan) — lihat
/// `DownloadEngine::get_all_summaries`.
...
mod tests {
    ...
    fn batch_action_tracks_deferred_retry_then_manual_pause() {
        // Error + retry tertunda → Pause. Setelah user pause manual, engine
        // menulis status=Paused & resume_pending=false → Resume. (Perilaku
        // `request_pause` yang membersihkan resume_pending diuji di types.rs.)
